...
bootstrap-config:
    ...
    @echo "╚══════════════════════════════════════════════════════════╝"
    @echo ""
    @echo "SSH keys must come before anything else (all git remotes use"
    @echo "git@github.com). Two modes: network (clone/reinstall) or"
    @echo "direct rsync over LAN for bulk data."
    @echo ""
    @echo "─── Must copy manually (always rsync) ───"
    @echo "  ~/.ssh/   (rsync -a user@old-mac:.ssh/ ~/.ssh/)"
    @echo "  ~/.gnupg/ (rsync -a user@old-mac:.gnupg/ ~/.gnupg/)"
    @echo ""
    @echo "─── Git clone (after SSH keys) ───"
    @echo "  Password store:"
    ...
    @echo "      ~/.config/nvim"
    @echo ""
    @echo "─── Reinstall via curl (faster than rsync on good internet) ───"
    @echo "  fnm:"
    @echo "    curl -fsSL https://fnm.vercel.app/install | bash"
    @echo "    fnm install --lts"
    @echo "  bun (~4 GB with cache):"
    ...
    @echo ""
    @echo "─── Bulk data (rsync over LAN) ───"
    @echo "  ~/Projects/ (~67 GB):"
    @echo "    rsync -aP --progress user@old-mac:~/Projects/ ~/Projects/"
    @echo "  ~/Library/Application Support/kiro-cli/ (~60 MB):"
    @echo '    rsync -aP "user@old-mac:~/Library/Application Support/kiro-cli/"'
    @echo '      "~/Library/Application Support/kiro-cli/"'
    @echo ""
    @echo "─── One-shot migration (after SSH keys) ───"
    @echo "  rsync -aP --progress user@old-mac:~/Projects/ ~/Projects/ \\"
    @echo '    "user@old-mac:~/Library/Application Support/kiro-cli/" \\'
    @echo '    "~/Library/Application Support/kiro-cli/"'
    @echo ""
    @echo "For Projects, you can git clone individual repos instead of 67 GB."
